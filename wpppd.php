...
add_shortcode('wpppd', 'wpppd_shortcodes');
...
function wpppd_install(){
	$getTable=mysql_query("SHOW TABLES LIKE '".DONATIONS_TABLE."'");

	if(!@mysql_result($getTable,0,0)){
		mysql_query("CREATE TABLE ".DONATIONS_TABLE." (`id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,`date` INT NOT NULL ,`name` VARCHAR( 255 ) NOT NULL ,`currency` VARCHAR( 25 ) NOT NULL ,`amount` FLOAT NOT NULL ,`email` VARCHAR( 255 ) NOT NULL ,`link` VARCHAR( 255 ) NOT NULL ,`display` VARCHAR( 255 ) NOT NULL)");
...
function wpppd_options(){
	...
	<tr valign="top"><td><b>'.__('Target?',$WPLD_Domain).'</b></td>';
	echo '<td><input name="wpppd_showtarget" type="text" value="'.get_option('wpppd_showtarget').'"></td>
	<td>'.__('Enter an amount to display donations as a % of your target figure. This works best with Show Totals set to No. Leave this blank to disable.<br/><i>Note: If you receive donations in multiple currencies this may become innacurate.</i>',$WPLD_Domain).'</td>
	</tr>

	<tr valign="top"><td><b>'.__('Show Totals?',$WPLD_Domain).'</b></td>';

	unset($Yes,$No);
	if(get_option('wpppd_showtotals')=='No'){
		$No=' SELECTED';
	}else{
		$Yes=' SELECTED';
	}

	echo '<td><select name="wpppd_showtotals"><option value="Yes"'.$Yes.'>'.$_Yes.'</option><option value="No"'.$No.'>'.$_No.'</option></select></td>
	<td>'.__('Show or hide the total amounts donated so far.',$WPLD_Domain).'</td>
	</tr>
	...
	<input type="hidden" name="page_options" value="wpppd_page,wpppd_email,wpppd_numtoshow,wpppd_showtotals,wpppd_showtarget,wpppd_datetoshow,wpppd_currency,wpppd_incentive,wpppd_linkcost,wpppd_testmode,wpppd_poweredby" />
...
function wpppd_display(){
	...
	case 'success':// Order was successful... 
		...
		if($_POST['mc_gross']>0 && $_POST['mc_currency'] && $_POST['payer_email']){
			$now=time();
			mysql_query("INSERT INTO ".DONATIONS_TABLE." VALUES ('',$now,'{$_POST['first_name']} {$_POST['last_name']}','{$_POST['mc_currency']}','{$_POST['mc_gross']}','{$_POST['payer_email']}','{$_POST['option_selection2']}','{$_POST['option_selection1']}')")or die(mysql_error());
	...
	if(get_option('wpppd_showtarget')>0){
		$Currency=get_option('wpppd_currency');
		$getTotal=mysql_query("SELECT SUM(amount) as amount FROM ".DONATIONS_TABLE);
		$TotalNum=money_format('%i',mysql_result($getTotal,0,0));

		$Target=get_option('wpppd_showtarget');

		$Percent=intval($TotalNum/$Target*100);
		$Width=$Percent*3.5;

		echo 'We have reached '.$Currencies[$Currency].$TotalNum.' ('.$Percent.'%) of our target of '.$Currencies[$Currency].get_option('wpppd_showtarget').'!<br/>
		<div style="border:thin solid #000;width:350px;height:15px;background-color:#fff;"><div style="text-align:center;width:'.$Width.'px;height:15px;color:#ddd;font-weight:bold;background-color:green;">'.$Percent.'%</div></div><br/>';

	}

	// Show Totals

	if(get_option('wpppd_showtotals')=='Yes'){
	...
	if($Incentive!='Do Not Offer Incentive'){
		echo 'If you donate a value of '.$Currencies[$Currency].(get_option('wpppd_linkcost') ? get_option('wpppd_linkcost') : '0.00') .' '.$Currency.' or more then we will create a link to a site of your choice. For this to work you must click the Return button on the PayPal page after you have completed the donation.<br><br>';
	}

	// Show Donate Button

	echo '<div align="center" style="background:#eee;border:thin solid black;" border="1">
	<form action="'.$_SERVER['REQUEST_URI'].'?action=process" method="post">
	<table style="text-align:left"><tr><td>Display Preference:</td><td><select name="os0" style="width:230px;"><option value="1">Show My Name And Amount</option><option value="2">Show My Name, Hide The Amount</option><option value="3">Hide My Name And Amount</option></select></td></tr>';

	if($LinkCost!='Do Not Offer Links'){
		echo '<tr><td>Optional URL:<input type="hidden" name="on1" value="Optional URL"/></td><td><input type="text" name="os1" value=""></td></tr>';
	}
	...
	$getDonations=mysql_query("SELECT * FROM ".DONATIONS_TABLE." {$DateToShow} ORDER BY date DESC{$NumToShow}")or die(mysql_error());

	if(mysql_num_rows($getDonations)>0){
		echo '<br/><table width="100%" style="border-spacing:5px;"><tr><th>Name</th><th width="100">Donation</th>'.( $Incentive!='Do Not Offer Incentive' ? '<th>Link</th>' : '' ).'</tr>';

		while($Donation=mysql_fetch_assoc($getDonations)){
			$x++;
			echo "<tr ".(is_int($x/2)? 'style="background-color:#f7f7f7"' : '' )."><td>{$Donation['name']} </td><td>{$Currencies[$Donation['currency']]}".number_format($Donation['amount'],2)." {$Donation['currency']}</td>".( $Incentive!='Do Not Offer Incentive' ? '<td>'.($Donation['amount'] > $LinkCost && $Donation['link'] ? '<a href="'.$Donation['link'].'">'.$Donation['link'].'</a>' : '').'</td>' : '')."</tr>";
		}
